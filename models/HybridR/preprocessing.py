...
from utils.hybridR_utils import *
import re
import random
import nltk
import dateparser
...
resource_path = 'inputs'
base_path = 'models/HybridR'
best_threshold = 0.80
...
def preprocessing_main():
    with open(f'{base_path}/test_inputs/test.json', 'r') as f:
        test_data = json.load(f)
    print(test_data)
    if not os.path.exists(f'{base_path}/preprocessed_test'):
        os.mkdir(f'{base_path}/preprocessed_test')
    
    from multiprocessing import Pool

    if not os.path.exists(f'{base_path}/preprocessed_test/test_step1.json'):
        ...
        with open(f'{base_path}/preprocessed_test/test_step1.json', 'w') as f:
            json.dump(results, f, indent=2)

    if not os.path.exists(f'{resource_path}/tables_tmp'):
        os.mkdir(f'{resource_path}/tables_tmp')

    if not os.path.exists(f'{base_path}/preprocessed_test/test_step2.json'):
        print("building CELL step for test data")

        with open(f'{base_path}/preprocessed_test/test_step1.json', 'r') as f:
            data = json.load(f)
        ...
        with open(f'{base_path}/preprocessed_test/test_step2.json', 'w') as f:
            json.dump(results, f, indent=2)

    if not os.path.exists(f'{base_path}/preprocessed_test/test_inputs.json'):
        print("constructing test inputs")

        with open(f'{base_path}/preprocessed_test/test_step2.json', 'r') as f:
            data = json.load(f)
        results = generate_inputs(data)
        with open(f'{base_path}/preprocessed_test/test_inputs.json', 'w') as f:
            json.dump(results, f, indent=2)